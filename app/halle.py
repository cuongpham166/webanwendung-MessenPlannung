...
class Halle_cl(object):

	def __init__(self):
		self.db_halle = Database_halle()
		self.view_o = View_cl()
	...
	def DELETE (self, id):
		...
		retVal_s = ''
		#db_halle.saveData_p()
		self.db_halle.delete_px(id)
		retVal_s = self.getList_p()
		return retVal_s

	def POST (self, data_opl):
		retVal_s = ''
		data_opl['id'] = str(self.db_halle.data_o_count)
		self.db_halle.create_px(data_opl)
		retVal_s = self.getList_p()
		return retVal_s
	...
	def PUT (self, data_opl):
		...
		id = data_o['id']
		self.db_halle.update_px(data_o, id)
		retVal_s = self.getList_p()
		return retVal_s

	def getList_p(self):
		data_a = self.db_halle.read_px()
		ndata_a = data_a[1:]
		return self.view_o.createList_px(ndata_a)

	def getDetail_p(self, id_spl):
		data_o = self.db_halle.read_px(id_spl)
